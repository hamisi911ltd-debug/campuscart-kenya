// Cloudflare Pages Function - manage team member admin accounts and their
// permissions. Owner-only: uses the same isAdmin() check as every other
// non-product admin endpoint, so a team member's own session can't reach
// this at all (it's never checked here).
import { enforceAdminDomain } from "../_lib/adminDomain";
import { ensureTeamTables, hashPassword, sanitizePermissions } from "../_lib/teamAuth";
...
export async function onRequestGet(context: { env: Env; request: Request }) {
  ...
  try {
    ...
    const { results } = await env.DB.prepare(
      "SELECT id, email, full_name, permissions, is_active, created_at FROM admin_team ORDER BY created_at DESC"
    ).all<{ id: string; email: string; full_name: string | null; permissions: string; is_active: number; created_at: string }>();

    const team = (results || []).map((row) => {
      let permissions: string[] = [];
      try { permissions = sanitizePermissions(JSON.parse(row.permissions)); } catch { /* default empty */ }
...
export async function onRequestPost(context: { env: Env; request: Request }) {
  ...
  try {
    await ensureTeamTables(env);
    const body = await request.json() as { email?: string; password?: string; full_name?: string; permissions?: string[] };
    ...
    await env.DB.prepare(
      "INSERT INTO admin_team (id, email, password_hash, full_name, permissions, is_active) VALUES (?, ?, ?, ?, ?, 1)"
    ).bind(id, email, passwordHash, body.full_name || null, JSON.stringify(permissions)).run();
...
export async function onRequestPut(context: { env: Env; request: Request }) {
  ...
  try {
    const body = await request.json() as { id?: string; is_active?: boolean; permissions?: string[] };
    if (!body.id) {
    ...
    if (Array.isArray(body.permissions)) {
      const permissions = sanitizePermissions(body.permissions);
      await env.DB.prepare("UPDATE admin_team SET permissions = ? WHERE id = ?").bind(JSON.stringify(permissions), body.id).run();
    }
